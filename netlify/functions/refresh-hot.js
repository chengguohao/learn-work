...
// 前端点击刷新 → 调这个函数 → 抓抖音API → 写入 Supabase
...
exports.handler = async function () {
  try {
    ...
    if (items.length === 0) {
      return { statusCode: 500, body: '未抓取到热点数据' };
    }
    ...
    // 清空旧数据
    await fetch(SUPABASE_URL + '/rest/v1/douyin_hot', {
    ...
    if (!ins.ok) {
      ...
      if (txt.includes('row-level-security')) {
        return {
          statusCode: 403,
          body: '需要在 Supabase 中运行 setup-rls.sql 开启写入权限'
        };
      }
      return { statusCode: 500, body: '写入 Supabase 失败' };
    }
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        count: items.length,
        data: records.map(r => ({ title: r.title, views: r.views }))
      })
    };
  } catch (e) {
